...
*2022年5月12日已完成之前所有需要处理的面试实际问题* 

### 未知
#### 找出连续三天及三天以上都有营业额，且营业额都在上涨的店铺  
1. 输出每天营业额，按时间倒序  
2. 然后侧视图排序1.2.3..4 取出小于等于3的数据  
3. 这个的结果出来后行专列，生成店铺名name，第一天额度day1，第二天额度day2，第三天额度day3 等四列  
4. 然后where条件day3 > day2 > day1  
...
### 中软1. 自我介绍，包括项目介绍和业务介绍
2. 你在项目中的角色、用到的技术、做的工作 3. 业务的数据量（每天的数据量有多少）4. Flume的源头数据5. 你是如何做数据清洗的6. 数仓的整体架构7. Sqoop的一些参数介绍8. Hive的窗口函数9. 建模经验介绍一下
### 未知
1. 业务分析具体讲一讲：什么需求，具体怎么得到2. 数据采集：来源+工具+使用中是否遇到问题3. 数仓搭建4. 埋点数据缺失怎么处理，埋点数据相关的表示如何设计的5. sql执行错误报警机制6. 项目数据量，日活，漏斗分析（要体现自己数据分析挖掘能力）
### 上海博彦科技1. Spark的序列化2. Java的io流分类3. Spark的算子分类4. Scala的case class和class的区别5. Scala的option类型6. Scala的泛型斜变逆变7. Scala的函数柯里化
8. Scala的隐式函数9. Flink-cdc10. 数仓开发指标编写11. MySQL存储过程12. Flink和Spark的区别

### 未知
1. Hive有任务跑的时间比较长，怎么优化 
2. Hive参数设置 
3. Hive什么时候走MapJoin
4. 什么情况下会产生Hive小文件
5. 小文件的危害
6. 一条sql语句，多个字段，如何考虑写尽量提升效率 
7. Hive日志如何反查哪条sql执行较慢
8. 数仓分层，自己负责哪些模块
9. MapReduce的过程 
10. Hive的查询过程，哪些方法可以提高查询效率
11. Hive常用的窗口函数 
12. Redis支持的数据类型
13. Spark Streaming怎么保证精准的消费 
14. Java怎么写事务
#### 姓名、学历、分数，统计总分前六名
...
3. 偏应用函数 
4. Scala 的闭包 
5. Scala 的 object 是什么 
6. Scala 中的隐式装换 
7. Scala 的 String 是可变的吗
8. 离线任务有多少个及任务执行时间
9. 数据有多少个指标 
10. hive 中的行列转换，除了 case when 的其他方法
11. hivesql 中常见的函数，窗口函数的区别 Rownumber 、 rank 、 dense rank explode 炸裂函数 
12. hive 的内部表和外部表的区别
13. 内部表和外部表如果删除，元数据是由谁来维护（ metastore ) 
14. hive 的元数据理解，他的存储位置及管理者是谁？
15. 分区和分桶的区别，分桶的原理
16. 分区可以提高查询效率吗？是不是分区越多越好 
17. hive 任务处理过程中遇到的问题 
18. spark 的转换算子和行动算子 
19. spark 的 Stage 和 task 的划分 
20. spark 的宽窄依赖

### 未知
1. 简单介绍一下你的业务 
2. 数仓的人员分配 
3. 你们的主题是根据什么来划分，为什么这么划分 
4. 你们源数据大概多少张表 
5. 你们的日志数据到建立事实表的过程中，主要做了什么
6. 你们的维度层是怎么建设的 
7. 如果在解析日志文件时遇到很多的硬编码，如何使用维度去解决 
8. 如果指标出现同义不同名的情况如何解决  
9. 阿里的 oneData 体系有了解吗 
10. 你们如何保证数据质量的 
11. 你日常遇到最多的数据质量问题是什么？ 
12. 你们的需求周期一般是多久？拿到需求之后怎么分析 
13. 在完成需求的过程中，有没有考虑过数仓的通用性．你们是如何体现的 
14. 有做过用户路径模型吗？每条路径的转化率是多少 
15. 你在工作中的最大收获是什么，带给你什么样的能力 
16. 你们的工作强度怎么样，能接受加班吗？ 
17. 如果一周内让你做＋个紧急的需求，你会怎么办？ 
18. 你们部门之间是直接进行对接吗？  
19. 有没有反驳过产品提出的需求，如果不合理怎么办？ 
20. 你在实时开发的过程中遇到什么问题．如何解决的 
21. 你们的实时模型是如何进行优化的，怎么评估它是否是一个优质的模型 
22. 你们有有过 clickhouse 做 join 吗，如何保证秒级延迟
23. flink的cep了解么？怎么使用的 
24. 你们的任务监控有做过吗，主要监控什么 
25. 你们如何保证数据的准确性
...
#### scala 是否可以多继承
Scala 中的多重继承由特质（trait）实现并遵循线性化规则。
